...
package com.example.admin.testcoordinatorlayoutfresh.tab;
...
public class TabLayout extends HorizontalScrollView {
    ...
    int indicatorWidth;
    boolean drawRound, indicatorAnimation;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        indicatorWidth = ma.getDimensionPixelSize(
              R.styleable.TabLayout_indicatorWidth, 0);
        drawRound = ma.getBoolean(
               R.styleable.TabLayout_indicatorDrawRound, false);
        indicatorAnimation = ma.getBoolean(
               R.styleable.TabLayout_indicatorTransAnimation, false);
        ...
        System.out.println("--" + indicatorWidth + "--" + drawRound+"---"+indicatorAnimation);
...
//        }
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                left = selectedTitle.getLeft() + (selectedTitle.getWidth() - indicatorWidth) / 2;
                ...
                if (indicatorAnimation){
                    if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                        ...
                        if (mSelectionOffset <= 0.5f) {
                            left = selectedTitle.getLeft()+ (selectedTitle.getWidth() - indicatorWidth) / 2;
                            right = AnimationUtils.lerp(right, nextTitle.getRight() - (selectedTitle.getWidth() - indicatorWidth) / 2, mSelectionOffset * 2);
                        } else {
                            left = AnimationUtils.lerp(left, nextTitle.getLeft()+(selectedTitle.getWidth() - indicatorWidth) / 2, (mSelectionOffset - 0.5f) * 2);
                            right = nextTitle.getRight()-(selectedTitle.getWidth() - indicatorWidth) / 2;
                        }
                    }
                }else {
                    if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                        // Draw the selection partway between the tabs
                        View nextTitle = getChildAt(mSelectedPosition + 1);
                        left = (int) (mSelectionOffset * (nextTitle.getLeft() + (nextTitle.getWidth() - indicatorWidth) / 2)
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            //这里处理点击tab 切换位置
            if (indicatorAnimation) {
                final int targetLeft = targetView.getLeft()+(targetView.getWidth() - indicatorWidth) / 2;
                final int targetRight = targetView.getRight() - (targetView.getWidth() - indicatorWidth) / 2;
                final int startLeft;
                ...
                final int targetLeft = targetView.getLeft() + (targetView.getWidth() - indicatorWidth) / 2;
                final int targetRight = targetLeft + indicatorWidth;
                final int startLeft;
                final int startRight;
...
//            canvas.restoreToCount(sc);
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                LinearGradient linearGradient = new LinearGradient(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight - mIndicatorMarginBottom
                        , mIndicatorRight, getHeight() - mIndicatorMarginBottom, mStartColor
                        , mEndColor, Shader.TileMode.CLAMP);
                mSelectedIndicatorPaint.setShader(linearGradient);
                if (!drawRound) {
                    canvas.drawRect(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight - mIndicatorMarginBottom,
                            mIndicatorRight, getHeight() - mIndicatorMarginBottom, mSelectedIndicatorPaint);
                }else {

                    RectF oval3 = new RectF(mIndicatorLeft, getHeight() - mSelectedIndicatorHeight - mIndicatorMarginBottom,
                            mIndicatorRight, getHeight() - mIndicatorMarginBottom);
                    canvas.drawRoundRect(oval3,mSelectedIndicatorHeight / 2, mSelectedIndicatorHeight / 2,mSelectedIndicatorPaint);
                }
            }
